...
AT(.text.display.ledseg)
void ledseg_disp_rec_playtime(void)
{
#if REC_AUTO_PLAY
    ledseg_buf[0] = ledseg_num_table[rec_play_cb.min / 10];
    ledseg_buf[1] = ledseg_num_table[rec_play_cb.min % 10];
    ledseg_buf[2] = ledseg_num_table[rec_play_cb.sec / 10];
    ledseg_buf[3] = ledseg_num_table[rec_play_cb.sec % 10];
    ledseg_buf[4] |= ICON_DDOT | ICON_PLAY;
    if (sys_cb.cur_dev <= DEV_SDCARD1) {
        ledseg_buf[4] |= ICON_SD;
    } else if (sys_cb.cur_dev == DEV_UDISK) {
        ledseg_buf[4] |= ICON_USB;
...
AT(.text.display.ledseg)
void ledseg_disp_rec_pause(void)
{
#if REC_AUTO_PLAY
    ledseg_buf[0] = T_LEDSEG_P;
    ledseg_buf[1] = T_LEDSEG_A;
...
AT(.text.display.ledseg)
void ledseg_disp_spdif(void)
{
    if (is_mute_flicker()) {
        return;
    }
    ledseg_buf[1] = T_LEDSEG_S;
    ledseg_buf[2] = T_LEDSEG_P;
    ledseg_buf[3] = T_LEDSEG_F;
}
...
const PFUNC ledseg_disp_pfunc[] = {
    ...
    ledseg_disp_rectime,
    ledseg_disp_rec_playtime,
    ledseg_disp_rec_pause,
    ledseg_disp_spdif,
    ledseg_disp_fmam,
    ledseg_disp_speaker,
    ledseg_disp_playdev,
